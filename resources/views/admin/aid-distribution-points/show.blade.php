    ...
    <div class="px-4 py-5 sm:p-6">
        <div class="sm:flex sm:items-center mb-6">
            <div class="sm:flex-auto">
                <h3 class="text-lg font-medium leading-6 text-gray-900">Detail Titik Distribusi Bantuan</h3>
            </div>
            <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                <a href="{{ route('admin.aid-distribution-points.edit', $aidDistributionPoint) }}" class="inline-flex items-center justify-center rounded-md border border-transparent bg-yellow-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500">
        ...
        <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
            <div>
                ...
                <dd class="mt-1 text-sm text-gray-900">{{ $aidDistributionPoint->id }}</dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">Nama</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $aidDistributionPoint->name }}</dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">Tipe Bantuan</dt>
            ...
            <div>
                ...
                <dd class="mt-1 text-sm text-gray-900">{{ $aidDistributionPoint->description ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">Alamat</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $aidDistributionPoint->address ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">Kontak Person</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $aidDistributionPoint->contact_person ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">No. Telepon</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $aidDistributionPoint->contact_phone ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">Kapasitas per Hari</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ number_format($aidDistributionPoint->capacity_per_day ?? 0) }} orang</dd>
            </div>
            <div>
            ...
            <div class="sm:col-span-2">
                ...
                <dd class="mt-1 text-sm text-gray-900">
                    <pre class="bg-gray-50 p-4 rounded-md text-xs overflow-x-auto">{{ json_encode($aidDistributionPoint->point_coordinates, JSON_PRETTY_PRINT) }}</pre>
                </dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">Dibuat</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $aidDistributionPoint->created_at->format('d/m/Y H:i:s') }}</dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">Diperbarui</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $aidDistributionPoint->updated_at->format('d/m/Y H:i:s') }}</dd>
            </div>
        </dl>
