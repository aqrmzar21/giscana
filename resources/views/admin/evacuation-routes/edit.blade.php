...
<div class="bg-white shadow rounded-lg">
    <div class="px-4 py-5 sm:p-6">
        <h3 class="text-lg font-medium leading-6 text-gray-900 mb-6">Form Edit Rute Evakuasi</h3>
        ...
        <form action="{{ route('admin.evacuation-routes.update', $evacuationRoute) }}" method="POST">
            ...
            <div class="space-y-6">
                    ...
                    <div class="mt-1">
                        <select name="evacuation_facility_id" id="evacuation_facility_id" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('evacuation_facility_id') border-red-300 @enderror">
                            ...
                            @foreach($facilities as $f)
                                <option value="{{ $f->id }}" {{ old('evacuation_facility_id', $evacuationRoute->evacuation_facility_id) == $f->id ? 'selected' : '' }}>{{ $f->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <select id="route_type" name="route_type" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('route_type') border-red-300 @enderror">
                                ...
                                <option value="primary" {{ old('route_type', $evacuationRoute->route_type) === 'primary' ? 'selected' : '' }}>Utama</option>
                                <option value="secondary" {{ old('route_type', $evacuationRoute->route_type) === 'secondary' ? 'selected' : '' }}>Sekunder</option>
                                <option value="emergency" {{ old('route_type', $evacuationRoute->route_type) === 'emergency' ? 'selected' : '' }}>Darurat</option>
                            </select>
                            @error('route_type')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <textarea id="description" name="description" rows="3" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('description') border-red-300 @enderror">{{ old('description', $evacuationRoute->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="number" step="0.01" name="length_km" id="length_km" value="{{ old('length_km', $evacuationRoute->length_km) }}" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('length_km') border-red-300 @enderror">
                            @error('length_km')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <textarea id="line_coordinates" name="line_coordinates" rows="5" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md font-mono text-xs @error('line_coordinates') border-red-300 @enderror">{{ old('line_coordinates', json_encode($evacuationRoute->line_coordinates, JSON_PRETTY_PRINT)) }}</textarea>
