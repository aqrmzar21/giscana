...
@section('title', 'Tambah Data Penerima Bantuan - Admin')

@section('page-title', 'Tambah Data Penerima Bantuan')
...
<div class="bg-white shadow rounded-lg">
    <div class="px-4 py-5 sm:p-6">
        <h3 class="text-lg font-medium leading-6 text-gray-900 mb-6">Form Tambah Data Penerima Bantuan</h3>
        
        <form action="{{ route('admin.aid-recipients.store') }}" method="POST">
            @csrf

            @php
                $selectedVillageId = old('village_id', '');
            ...
            <div class="space-y-6">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="date" name="date" id="date" value="{{ old('date', date('Y-m-d')) }}" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('date') border-red-300 @enderror">
                            @error('date')
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="text" name="recipient_name" id="recipient_name" value="{{ old('recipient_name') }}" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('recipient_name') border-red-300 @enderror">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div class="mt-1">
                            <select id="aid_type" name="aid_type" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('aid_type') border-red-300 @enderror">
                                ...
                                @foreach(['Sembako', 'Material Bangunan', 'Pakaian Layak Pakai', 'Obat-obatan'] as $type)
                                    <option value="{{ $type }}" {{ old('aid_type') == $type ? 'selected' : '' }}>{{ $type }}</option>
                                @endforeach
                            </select>
                            @error('aid_type')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="number" name="amount" id="amount" value="{{ old('amount') }}" min="0" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('amount') border-red-300 @enderror">
                            @error('amount')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <textarea id="description" name="description" rows="3" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-300 rounded-md @error('description') border-red-300 @enderror">{{ old('description') }}</textarea>
